        ...
        if _est_jour_de_paiement(contrat, aujourd_hui):
            ...
            if not deja_paye:
                ...
                paiements_attendus.append({
                    ...
                    'heure_limite': timezone.now().replace(hour=12, minute=0, second=0),
                    ...
                    try:
                        if type_contrat == 'chauffeur':
                            ...
                            if not penalite_existe:
                                ...
                                type_penalite = 'combine' if ContratBatterie.objects.filter(chauffeur=contrat.chauffeur,
...
def appliquer_penalites_view(request):
    """Applique automatiquement les pénalités de retard pour tous les contrats actifs"""
    aujourd_hui = date.today()
    ...
    # 1. Pénalités pour les paiements du jour même après 12h ou 14h
    # Contrats chauffeur
    ...
    # Contrats partenaire
    for contrat in ContratPartenaire.objects.filter(statut='actif'):
        if _est_jour_de_paiement(contrat, aujourd_hui):
            deja_paye = Paiement.objects.filter(contrat_partenaire=contrat, date_paiement=aujourd_hui,
                                                est_penalite=False).exists()
            penalite_existe = Penalite.objects.filter(
                contrat_partenaire=contrat,
                date_paiement_manque=aujourd_hui
            ).exists()
    ...
    for contrat in ContratBatterie.objects.filter(statut='actif'):
        # Vérifier si c'est un contrat batterie "standalone" (pas lié à un contrat chauffeur/partenaire)
        is_standalone = True

        if contrat.chauffeur:
            # Vérifier si le chauffeur a un contrat actif
            if ContratChauffeur.objects.filter(association__validated_user=contrat.chauffeur, statut='actif').exists():
                is_standalone = False
        ...
        if is_standalone and _est_jour_de_paiement(contrat, aujourd_hui):
            deja_paye = Paiement.objects.filter(contrat_batterie=contrat, date_paiement=aujourd_hui,
                                                est_penalite=False).exists()
            penalite_existe = Penalite.objects.filter(
                contrat_batterie=contrat,
                date_paiement_manque=aujourd_hui
    ...
    # 2. Pénalités pour les paiements manqués des jours précédents
    jours_max_a_verifier = 30  # Vérifier les 30 derniers jours maximum

    # Contrats chauffeur
    for contrat in ContratChauffeur.objects.filter(statut='actif'):
        for jour in range(1, jours_max_a_verifier + 1):
            jour_a_verifier = aujourd_hui - timedelta(days=jour)

            # Vérifier si c'était un jour de paiement
            if _est_jour_de_paiement(contrat, jour_a_verifier):
                # Vérifier si le paiement a été effectué
                paiement_effectue = Paiement.objects.filter(
                    contrat_chauffeur=contrat,
                    date_paiement=jour_a_verifier,
                    est_penalite=False
                ).exists()

                # Vérifier si une pénalité existe déjà pour ce jour
                penalite_existe = Penalite.objects.filter(
                    contrat_chauffeur=contrat,
                    date_paiement_manque=jour_a_verifier
                ).exists()

                # Si le paiement n'a pas été effectué et aucune pénalité n'existe, en créer une
                if not paiement_effectue and not penalite_existe:
                    jours_retard = jour
                    type_penalite = 'combine' if ContratBatterie.objects.filter(chauffeur=contrat.association.validated_user,
                                                                                statut='actif').exists() else 'batterie_seule'

                    # Calculer le montant de la pénalité en fonction du nombre de jours de retard
                    montant, motif = ReglePenalite.get_penalite_applicable(type_penalite, None, jours_retard)
                    ...
                    if montant > 0:
                        Penalite.objects.create(
                            ...
                            montant=montant,
                            motif='retard_paiement',
                            description=f"Pénalité pour paiement manqué du {jour_a_verifier.strftime('%d/%m/%Y')} - {jours_retard} jours de retard",
                            statut='en_attente',
                            date_paiement_manque=jour_a_verifier,
                            cree_par=user
                        )
    ...
    for contrat in ContratBatterie.objects.filter(statut='actif'):
        ...
        if contrat.chauffeur:
            if contrat.chauffeur and ContratChauffeur.objects.filter(association__validated_user=contrat.chauffeur,
                                                                     statut='actif').exists():
                is_standalone = False
            ...
            for jour in range(1, jours_max_a_verifier + 1):
                ...
                if _est_jour_de_paiement(contrat, jour_a_verifier):
                    ...
                    if not paiement_effectue and not penalite_existe:
                        ...
                        # Calculer le montant de la pénalité en fonction du nombre de jours de retard
                        montant, motif = ReglePenalite.get_penalite_applicable('batterie_seule', None, jours_retard)
    ...
    messages.success(request, f"{penalites_creees} pénalité(s) ont été créées automatiquement.")
    return redirect('payments:gestion_penalites')
